...
public class MoneyToBig {
    ...
    public String moneytobig(Double moneys) {
        BigDecimal numberOfMoney = new BigDecimal(moneys);
        String s = MoneyToBig.number2CNMontrayUnit(numberOfMoney);
        System.out.println("你输入的金额为：【" + moneys + "】   #--# [" + s.toString() + "]");
        return s;
    }
     ...
     * @return
     */
    public static String convert2CNDesc(Double money) {
        if (null!=money) {
            BigDecimal numberOfMoney = new BigDecimal(String.valueOf(money));
            return MoneyToBig.number2CNMontrayUnit(numberOfMoney);
        }else {
            return null;
        }
    }
...
//         String s = MoneyToBig.number2CNMontrayUnit(numberOfMoney);
         String s = MoneyToBig.convert2CNDesc(money);
